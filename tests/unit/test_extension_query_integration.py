...
def test_ieee_compiler_preserves_current_project_query_contract():
    ...
    assert query == (
        '("large language models" OR "LLM") AND '
...
def test_site_contains_versioned_origin_local_query_bridge_without_fixed_limit():
    ...
    assert "LITSYNC_SCHOLAR_QUERY_CONTEXT" in html
    assert "LITSYNC_SCHOLAR_EXTENSION_READY" in html
    assert "LITSYNC_SCHOLAR_EXTENSION_PROBE" in html
    assert "query: queries.google_scholar || ''" in html
    assert "Open Google Scholar" in html
