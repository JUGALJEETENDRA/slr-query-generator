...
from litsync_app.integrations.gemini_web_screening_browser import (
    GeminiWebBrowser,
    GeminiWebTransportError,
)
...
def test_start_preserves_one_blank_anchor_page_for_persistent_context():
    ...
    browser = GeminiWebBrowser(playwright_factory=Factory)
...
def test_page_closes_when_submission_fails(monkeypatch):
    ...
    with pytest.raises(GeminiWebTransportError) as captured:
...
def test_browser_uses_async_playwright_and_dedicated_profile_source():
    import inspect
    import litsync_app.integrations.gemini_web_screening_browser as module
