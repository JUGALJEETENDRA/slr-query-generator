...
def test_results_are_json_safe_and_have_browser_download_url():
    SCREENING_SESSION.begin(
        "json-job", "outputs/runs/screened-json-job.csv", "local-cross-model-three-layer-v3"
    )
...
def test_progress_and_results_reject_mismatched_job_ids():
    ...
    PROGRESS.begin_screening(job_id, 3, "local-cross-model-three-layer-v3")
    SCREENING_SESSION.begin(
        job_id, f"outputs/runs/screened-{job_id}.csv", "local-cross-model-three-layer-v3"
    )
...
def test_csv_defaults_to_automatic_local_ai(monkeypatch):
    ...
    assert payload["architecture_version"] == "local-cross-model-three-layer-v3"
...
def test_manifest_backed_screening_is_restored_after_server_restart(monkeypatch, tmp_path):
    ...
    (tmp_path / "latest_screening.json").write_text(json.dumps({
        ...
        "architecture_version": "local-cross-model-three-layer-v3",
    }), encoding="utf-8")
