...
from evaluation.gemini_web_v2_validation import validate_diagnostics


SCHEMA_VERSION = "research-validation-v1"
FROZEN_GEMINI_VERSION = "gemini-web-batched-v2.3"
...
def initialize_study(
    *, corpus_path: str | Path, research_question: str, title_column: str,
    ...
    manifest = {
        ...
        "screening": {"engine": "gemini_web", "required_version": FROZEN_GEMINI_VERSION, "runs": []},
...
def run_study(
    ...
    _save_manifest(manifest, paths)
    for directory in (
        paths["public"] / "cache" / "gemini_web" / "diagnostics",
        paths["public"] / "cache" / "gemini_web" / "checkpoints",
    ):
    ...
    for label in ("repeat_a", "repeat_b"):
        ...
        if summary.get("architecture_version") != FROZEN_GEMINI_VERSION:
            raise RuntimeError("Research validation must use frozen Gemini Web v2.3")
...
def _load_protocol(paths: dict[str, Path], protocol_id: str) -> dict[str, Any]:
    protocol_root = paths["public"] / "cache" / "gemini_web" / "protocols"
