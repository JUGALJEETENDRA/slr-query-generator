...
function page(html, url = 'https://pubmed.ncbi.nlm.nih.gov/') {
...
function background() {
  const store = {}, tabs = new Map([[7, {id:7,url:'https://pubmed.ncbi.nlm.nih.gov/'}]]), downloads = [];
  let nextTab = 8;
  const chrome = {
  ...
  const state = () => structuredClone(store.litsync_pubmed_native_v1);
  return {chrome,store,tabs,downloads,send,state,ctx};
}
const payload = {schema_version:1, query_versions:{balanced:{pubmed:' ("All Metadata":"alpha") '},high_recall:{pubmed:'("All Metadata":"alpha" OR "All Metadata":"beta")'}},query_fingerprints:{balanced:'b',high_recall:'h'}};
