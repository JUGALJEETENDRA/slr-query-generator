...
function owner(s, m, sender) {
  const j = s.job;
  if (!j?.active || j.id !== m.id || j.tabId !== sender.tab?.id || !C.isIEEE(sender.url)
...
async function reconcile(s) {
  ...
  } else if (d.state === "interrupted") pause(s, "IEEE download was interrupted. Check browser Downloads; it will not be requested twice.");
  else if (d.state === "complete") pause(s, "IEEE download completed but could not be identified as CSV.");
...
async function route(m, sender) {
  ...
  if (["VIEW", "START", "RESUME", "DISCARD", "PAUSE"].includes(m.type)) {
    ...
    if (m.type === "START") {
      ...
      if (!C.isIEEE(tab?.url) || !tab.url.startsWith(C.COMMAND)) throw Error("Open IEEE Command Search, then click Start");
    ...
    if (!tab || !(C.sameQuery(tab.url, s.job.query) || tab.url?.startsWith(C.COMMAND))) {
      tab = await chrome.tabs.create({url: s.job.resultsUrl || C.COMMAND, active: true});
    }
  ...
  if (m.type === "CLAIM") {
    const j = s.job;
    if (!j?.active || j.tabId !== sender.tab?.id || !C.isIEEE(sender.url) || sender.frameId !== 0 || j.attempt) return null;
    ...
    if (j.owner && j.owner !== document && !(j.stage === "waiting_for_results" && sender.documentId && j.ownerDocument !== sender.documentId)) {
      pause(s, "IEEE document or extension restarted. Resume the preserved run."); await save(s, "Owner changed; explicit Resume required"); return null;
    }
  ...
  const transitions = {
    ...
    results_ready: ["opening_export"], opening_export: ["waiting_for_export_dialog"],
    waiting_for_export_dialog: ["downloading"]
  };
  ...
  if (m.stage === "downloading") await chrome.alarms.create(`ieee-download-${j.id}`, {delayInMinutes: 0.5});
  return j;
}
...
chrome.downloads.onCreated.addListener(item => serial(async () => {
  ...
  if (j.downloadId != null && j.downloadId !== item.id) {
    pause(s, "Multiple IEEE downloads detected. Inspect browser Downloads."); await save(s, j.reason); return;
  }
...
chrome.alarms.onAlarm.addListener(alarm => serial(async () => {
  const s = await read();
  if (alarm.name !== `ieee-download-${s.job?.id}` || !s.job?.attempt || s.job.stage === "complete") return;
...
chrome.tabs.onRemoved.addListener(id => serial(async () => {
  const s = await read(); if (s.job?.tabId === id && s.job.stage !== "complete") { pause(s, "IEEE tab closed. Resume to restore it."); await save(s); }
}).catch(() => {}));
