...
from gemini_web_v24_prompt import (
    V24Paper,
    build_primary_prompt,
    build_protocol_prompt,
    build_verification_prompt,
)
from local_ai.contracts import SCHEMA_VERSION, StrictModel
...
class V24AssessmentBatch(StrictModel):
    ...
    items: list[V24Assessment] = Field(min_length=1, max_length=GEMINI_WEB_V24_BATCH_SIZE)
...
def _assessment_cache_key(protocol_id: str, paper: V24Paper) -> str:
    return _fingerprint({
        "version": GEMINI_WEB_V24_CACHE_VERSION,
...
def _contract_key(input_fingerprint: str, protocol_id: str) -> str:
    return _fingerprint({
        "version": GEMINI_WEB_V24_VERSION,
        "input_fingerprint": input_fingerprint,
        "protocol_id": protocol_id,
        "batch_size": GEMINI_WEB_V24_BATCH_SIZE,
    })
...
def _validate_protocol_sources(
    ...
    if inclusion_criteria.strip() and not any(
        criterion.source == "user" for criterion in protocol.required_inclusion_criteria
    ):
        raise ValueError("explicit researcher inclusion criteria were not preserved")
    if exclusion_criteria.strip() and not any(
        criterion.source == "user" for criterion in protocol.exclusion_boundaries
    ):
        raise ValueError("explicit researcher exclusion criteria were not preserved")
    ...
    *,
    ...
    prompt = (
        build_verification_prompt(
            ...
            papers=papers,
            flags=flags or {},
            schema=V24AssessmentBatch.model_json_schema(),
        )
        if verification
    ...
    for attempt in range(2):
        try:
            ...
            parsed = V24AssessmentBatch.model_validate(
                parse_structured_model_output(raw, V24AssessmentBatch)
            )
    ...
    *,
    ...
    for key, paper in papers.items():
        ...
        if not paper.abstract.strip():
            ...
            rows[key] = _row(source, source_index, paper, protocol, result)
            continue
        ...
        if cached is None:
            ...
            continue
        source_index, source = sources[key]
        rows[key] = _row(source, source_index, paper, protocol, cached)
    ...
    try:
        ...
        for batch_number in range(primary_batches):
            ...
            for paper in batch:
                ...
                rows[paper.paper_id] = _row(source, source_index, paper, protocol, result)
            ...
            progress.update_batch(
                ...
                min(len(pending), (batch_number + 1) * GEMINI_WEB_V24_BATCH_SIZE),
            )
        ...
        verification_batches = (
            len(verification_keys) + GEMINI_WEB_V24_BATCH_SIZE - 1
        ) // GEMINI_WEB_V24_BATCH_SIZE
        ...
        progress.begin_batches(
            ...
            verification_batches, GEMINI_WEB_V24_BATCH_SIZE,
        )
        for batch_number in range(verification_batches):
            ...
            progress.update_batch(
                job_id, batch_number + 1,
                min(len(verification_keys), (batch_number + 1) * GEMINI_WEB_V24_BATCH_SIZE),
            )
            progress.update_counts(
                job_id, len(ordered), counts["keep"], counts["maybe"], counts["reject"],
            )
